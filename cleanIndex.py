...
def search_indexed(query_str):
    ...
    with ix.searcher() as searcher:
        ...
        for hit in results:
            ...
            url = title_to_urls.get(hit['file_path'].replace(CONFIG["txt_directory"]+"/","").replace("/transcription.txt",""), "URL not found")
...
def get_indexed_files():
    ...
    if os.path.exists(CONFIG["INDEXED_FILES_LIST"]):
        with open(CONFIG["INDEXED_FILES_LIST"], 'r') as f:
...
def mark_as_indexed(file_path):
    ...
    """
    with open(CONFIG["INDEXED_FILES_LIST"], 'a') as f:
        f.write(file_path + "\n")
